...
/**
 * @file
 * Contains \Drupal\om_pssp\Form\OmPsspUploadAbstractCodeForm.
 */
...
class OmPsspUploadAbstractCodeForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'om_pssp_upload_abstract_code_form';
  }
  ...
  public function buildForm(array $form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($proposal_q) {
      if ($proposal_data = $proposal_q->fetchObject()) {
        ...
        drupal_goto('powersystems/pssp/abstract-code');
        return;
      }
    } //$proposal_q
    else {
    ...
    $form['other_libraries'] = [
      '#type' => 'radios',
      '#title' => t('Other Modelica Libraries Used'),
      '#options' => [
        '1' => 'Yes',
        '0' => 'No',
      ],
      '#default_value' => $abstracts_q->other_libraries_used,
      '#required' => TRUE,
    ];
    ...
    $form['upload_an_abstract'] = [
      ...
      //'#required' => TRUE,
		'#description' => t(t('<span style="color:red;">Current File :</span> ' . $existing_uploaded_A_file->filename . '<br />' . t('<span style="color:red;">Allowed file extensions : ') . \Drupal::config('om_pssp.settings')->get('om_pssp_abstract_upload_extensions') . '</span>')),
    ];
    ...
    $form['upload_om_pssp_developed_process'] = [
      '#type' => 'file',
      '#title' => t('Upload the Project Files'),
      //'#required' => TRUE,
		'#description' => t('<span style="color:red;">Current File :</span> ' . $existing_uploaded_S_file->filename . '<br />Separate filenames with underscore. No spaces or any special characters allowed in filename.') . '<br />' . t('<span style="color:red;">Allowed file extensions : ') . \Drupal::config('om_pssp.settings')->get('om_pssp_project_files_extensions') . '</span>',
    ];
    $form['prop_id'] = [
      '#type' => 'hidden',
    ...
    $form['imp_note'] = [
      '#type' => 'item',
      '#markup' => t('Note: <span style="color:red;">*It is mandatory to keep the OpenIPSL as a sub-package within the submitted package.</span>'),
    ];
    /*
    ...
    $form['submit'] = [
      ...
      '#value' => t('Submit'),
      '#submit' => [
        'om_pssp_upload_abstract_code_form_submit'
        ],
    ];
    // @FIXME
    ...
    // $form['cancel'] = array(
    // 		'#type' => 'item',
    // 		'#markup' => l(t('Cancel'), 'powersystems/pssp/abstract-code')
    // 	);
  ...
  public function validateForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    if (isset($_FILES['files'])) {
      ...
      if (!$existing_uploaded_S_file) {
        if (!($_FILES['files']['name']['upload_om_pssp_developed_process'])) {
          $form_state->setErrorByName('upload_om_pssp_developed_process', t('Please upload the file.'));
        }
      } //!$existing_uploaded_S_file
      if (!$existing_uploaded_A_file) {
      ...
      if ($_FILES['files']['name']['upload_an_udc'] || $_FILES['files']['name']['upload_an_abstract'] || $_FILES['files']['name']['upload_om_pssp_developed_process']) {
        foreach ($_FILES['files']['name'] as $file_form_name => $file_name) {
          if ($file_name) {
            /* checking file type */
            if (strstr($file_form_name, 'upload_om_pssp_developed_process')) {
              $file_type = 'S';
            }
            else {
            ...
            switch ($file_type) {
              case 'S':
                $allowed_extensions_str = \Drupal::config('om_pssp.settings')->get('om_pssp_project_files_extensions');
                break;
              case 'A':
                $allowed_extensions_str = \Drupal::config('om_pssp.settings')->get('om_pssp_abstract_upload_extensions');
                break;
            } //$file_type
            ...
            if (!om_pssp_check_valid_filename($_FILES['files']['name'][$file_form_name])) {
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($form_state->getValue(['other_libraries']) == 1) {
      $other_libraries_used = 1;
    }
    else {
      $other_libraries_used = 0;
    }
    ...
    $proposal_id = $proposal_data->id;
    $query_s = "SELECT * FROM {om_pssp_submitted_abstracts} WHERE proposal_id = :proposal_id";
    ...
    if (!$query_s_result) {
      ...
      $query = "INSERT INTO {om_pssp_submitted_abstracts} (
	proposal_id,
	approver_uid,
	...
	is_submitted) VALUES (:proposal_id, :approver_uid, :abstract_approval_status, :other_libraries_used, :abstract_upload_date, :abstract_approval_date, :is_submitted)";
      $args = [
        ":proposal_id" => $proposal_id,
        ":approver_uid" => 0,
        ":abstract_approval_status" => 0,
        ":other_libraries_used" => $other_libraries_used,
      ...
      $query1 = "UPDATE {om_pssp_proposal} SET is_submitted = :is_submitted WHERE id = :id";
      $args1 = [
        ":is_submitted" => 1,
        ":id" => $proposal_id,
      ];
      ...
      $query = "UPDATE {om_pssp_submitted_abstracts} SET 
    ...
    foreach ($_FILES['files']['name'] as $file_form_name => $file_name) {
      if ($file_name) {
        ...
        if (strstr($file_form_name, 'upload_om_pssp_developed_process')) {
          $file_type = 'S';
        } //strstr($file_form_name, 'upload_om_pssp_developed_process')
        else {
          if (strstr($file_form_name, 'upload_an_abstract')) {
            $file_type = 'A';
          }
        ...
        switch ($file_type) {
          case 'S':
            if (file_exists($root_path . $dest_path_project_files . $_FILES['files']['name'][$file_form_name])) {
              ...
              if (move_uploaded_file($_FILES['files']['tmp_name'][$file_form_name], $root_path . $dest_path_project_files . $_FILES['files']['name'][$file_form_name])) {
                ...
                $query_ab_f = "SELECT * FROM om_pssp_submitted_abstracts_file WHERE proposal_id = :proposal_id AND filetype = 
				:filetype";
                $args_ab_f = [
                  ":proposal_id" => $proposal_id,
                  ":filetype" => $file_type,
                ...
                if (!$query_ab_f_result) {
                  $query = "INSERT INTO {om_pssp_submitted_abstracts_file} (submitted_abstract_id, proposal_id, uid, approvar_uid, filename, filepath, filemime, filesize, filetype, timestamp)
          VALUES (:submitted_abstract_id, :proposal_id, :uid, :approvar_uid, :filename, :filepath, :filemime, :filesize, :filetype, :timestamp)";
                  ...
                  $query = "UPDATE {om_pssp_submitted_abstracts_file} SET filename = :filename, filepath=:filepath, filemime=:filemime, filesize=:filesize, timestamp=:timestamp WHERE proposal_id = :proposal_id AND filetype = :filetype";
                  $args = [
                    ":filename" => $_FILES['files']['name'][$file_form_name],
    ...
    if (!drupal_mail('om_pssp', 'abstract_uploaded', $email_to, language_default(), $params, $from, TRUE)) {
